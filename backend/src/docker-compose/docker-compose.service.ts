...
export class DockerComposeService {
  ...
  private parseModrinthConfig(serverConfig: ServerConfig, env: any): void {
    ...
    serverConfig.modrinthModpackVersion = env.MODRINTH_VERSION ?? '';
    serverConfig.modrinthExcludeFiles = env.MODRINTH_EXCLUDE_FILES ?? '';
    serverConfig.modrinthForceSynchronize = env.MODRINTH_FORCE_SYNCHRONIZE === 'true';
    serverConfig.modrinthIgnoreMissingFiles = env.MODRINTH_IGNORE_MISSING_FILES ?? '';
  }
  ...
  async ensureHorizonsServerReadiness(serverId: string, proxyEnabled = false): Promise<void> {
    ...
    const expectedIgnoreMissing = getHorizonsModrinthIgnoreMissingFiles();
    const needsComposeUpdate =
      config.modrinthExcludeFiles !== expectedExclude ||
      config.modrinthIgnoreMissingFiles !== expectedIgnoreMissing ||
      !config.modrinthForceSynchronize;
    ...
    if (needsComposeUpdate) {
      await this.updateServerConfig(
        serverId,
        {
          ...
          modrinthIgnoreMissingFiles: expectedIgnoreMissing,
